...
recursive neural network의 목적은 문장 내 구조를 이루는 parse tree를 만들고 문장의 의미를 나타내는 compositional vector representation
...
기존 recurrent와 convolution과의 차이점을 살펴보자. <br>
recurrent는 구(phrase) 단위로 의미를 capture하는게 당연히 쉽지 않고 마지막 단어의 정보가 많이 전달되는 경향이 있다.
...
$$W$$는 모두 같은 값으로 적용된다.
![recursive](https://whikwon.github.io/images/NLP_recursive.png)
...
속도가 느려서 tree의 subset만 활용하는 PCFG를 사용한다고 한다. 여기에 기존에 사용되는 TreeRNN을 사용하면 SU-RNN 구조가 완성된다.
...
![SU-RNN_ex1](https://whikwon.github.io/images/NLP_su_rnn_example1.png)
아래는 문장을 최종적으로 vector representation한 결과가 어떤 분포를 나타내고 있는지 확인한 결과이다.
확인 문장과 주변에 가장 가까운 2개 문장을 비교했을 때 유사한 의미의 내용을 나타내는 것을 볼 수 있다.
![SU-RNN_ex2](https://whikwon.github.io/images/NLP_su_rnn_example2.png)
...
![MV-RNN](https://whikwon.github.io/images/NLP_mv_rnn.png)
이렇게 했을 때의 결과는 아래와 같은데 가장 중요한 점은 기존 RNN이 ***not~*** 구문의 해석이 어려웠는데 이를 잘 해냈다는 점이다.
![MV-RNN_result](https://whikwon.github.io/images/NLP_mv_rnn_result.png)

...
comments: true
---
- 2014년에 Ian Goodfellow가 발표한 GAN(Generative Adversarial Nets)에 대해 정리하였다.
...
GAN이 학습하는 방법은 아주 쉽게 풀어서 설명하고 있다.
...
$$ p_z(z) : noise\ 변수의\ prior분포(Gaussian),\ p_g : generator의\ x에\ 대한\ 분포, x: 실제\ data\
D(x; \theta_d) : multilayer\ perceptron(discriminator)\ with\ parameter\ \theta_d,\ G(z; \theta_g) : multilayer\ perceptron(generator)
\ with\ parameter\ \theta_g
$$
...
학습에 따른 $$p_{dat}, p_g$$의 변화를 잘 나타낸 논문 내 설명 그림이다. 보면서 어떤 식으로 변화할 지에 대한 직관을 얻도록 하자.
...
### 3.1 Global Optimality of $$p_g = p_data$$ <br>
...
**Theorem 1.** <br>
> *The global mnimum of the virtual training criterion $$C(G)$$ is achieved if and only if
...
$$C(G) 식을 적절히 변형시켜야 하는데 이때 $$KL, JSD$$가 사용된다.
$$\begin{align}C(G) &= -log(4) + KL \left( p_{data} || \frac{p_{data}~+ p_g}{2}\right) + KL \left( p_g|| \frac{p_{data}~+ p_g}{2}\right) \\ &= -log(4) + 2\cdot JSD(p_{data}||p_g) \end{align}$$
...
convex함수의 supremum(상한)에 대한 subderivative는  maximum에 대한 derivative도 당연히 포함할 것이다.
이를 수식으로 나타내면
 $$f(x) = sup_{\alpha \in A}f_{\alpha}(x)$$ $$f_{\alpha}(x)$$이고 $$x$$에 모든 $$\alpha$$에 대해 convex하면,
...
$$p_g$$ 분포를 optimize할 때 $$G(z;\theta_g)$$에서 $$\theta_g$$를 optimize하므로 증명이 완전히 적용되진 않는다고 한다.
...
2.
3. [유재준씨 블로그](http://jaejunyoo.blogspot.com/2017/01/generative-adversarial-nets-1.html)
